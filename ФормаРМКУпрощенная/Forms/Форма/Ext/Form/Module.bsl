...
Процедура ПриОткрытии(Отказ)
	// Устанавливаем по умолчанию, что чек является приходным
	Объект.ЭтоПриходныйЧек = Истина;
	
	ПараметрыРабочегоМестаПоУмолчанию = РабочееМестоКассираВызовСервера.ПолучитьКассуККМИТерминалПоУмолчанию();
	
	Объект.Кассир = ПользователиКлиент.ТекущийПользователь();
	Объект.Организация = ПараметрыРабочегоМестаПоУмолчанию.Организация;
	Объект.КассаККМ = ПараметрыРабочегоМестаПоУмолчанию.КассаККМ;
	
	Если ЗначениеЗаполнено(ПараметрыРабочегоМестаПоУмолчанию.КассаККМ) Тогда
		ПолучитьРекизитКассыККМ(ПараметрыРабочегоМестаПоУмолчанию.КассаККМ, Объект.ФискальныйРегистратор, Объект.Организация);
	КонецЕсли;
...
Функция СформироватьПараметрыЧекаПечати()
	...
	Если СпособОплаты = "Безналичными" Тогда
...
Функция СоздатьИПровестиДокументЧекВ1С(ОтчетККМ, НомерСменыККТ = "", НомерЧекаККТ = "", ВозвращаемаяСсылкаНаДокументЧекВ1С = "", ОписаниеОшибкиСозданияДокументаЧекВ1С = "")
	...
	Исключение
		// Игнорируем ошибку, если свойство Статус недоступно
	КонецПопытки;
...
Процедура ЗаполнитьОплаты(ОбщиеПараметры, СуммаЧека)
	...
	Если СпособОплаты = "Наличными" Тогда  		
